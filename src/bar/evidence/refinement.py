...
class SupportScorer:
    ...
    def score(self, edge: Edge) -> float:
        ...
        curated_available = "curated_score" in edge.metadata or n_curated > 0.0
        text_available = "text_score" in edge.metadata or bool(edge.provenance)

        curated_score = float(edge.metadata.get(
            "curated_score",
            min(1.0, n_curated / max(self.curated_source_cap, 1)),
        ))
        if "text_score" in edge.metadata:
            text_score = float(edge.metadata["text_score"])
        elif edge.provenance:
            text_score = float(max(item.score for item in edge.provenance))
        else:
            text_score = float(edge.support_score)

        if curated_available and text_available:
            score = self.curated_weight * curated_score + (1.0 - self.curated_weight) * text_score
        elif curated_available:
            score = curated_score
        elif text_available:
            score = text_score
        else:
            score = float(edge.support_score)
